...
function sortTreeItems(items: NavItem[], sortByKey?: string): NavItem[] {
  ...
  return items.map((node) => {
    if (node.kind === 'folder' && node.children) {
      return {
        ...node,
        children: sortTreeItems(sortItems(node.children, sortByKey), sortByKey),
      }
    }
    return node
  })
...
export function transformViewpointData(
  ...
  // 1. Filter by Status (case-insensitive and alias-aware)
  if (viewpoint.filterStatus && viewpoint.filterStatus !== 'All') {
    ...
    filteredExtracted = filteredExtracted.filter((entry) => {
      ...
      if (filter === 'completed' && itemStatus === 'ok') return true
      if (filter === 'wait for event' && itemStatus === 'warning') return true
      if (filter === 'failed' && itemStatus === 'danger') return true
      return false
    })
  }

  // 2. Filter by Folder Scope
  ...
  // 3. Filter Edges to matching items
  ...
  // 4. Universal Grouping Logic
  if (grouping !== 'Folder') {
    const groupMap = new Map<string, NavItem[]>()

    for (const entry of filteredExtracted) {
      const rawVal =
        entry.item.data?.[grouping] ??
        (entry.item as unknown as Record<string, unknown>)[grouping] ??
        'Unassigned'
      ...
      groupMap.get(groupVal)!.push({
        ...
        data: {
          ...entry.item.data,
          folder: `${grouping} / ${groupVal}`,
        },
      })
    }

    transformedTree = Array.from(groupMap.entries()).map(([groupVal, items]) => {
      const sortedChildItems = sortItems(items, viewpoint.sortBy)
      return {
        id: `group-${grouping}-${groupVal.replace(/[^a-zA-Z0-9_-]/g, '_')}`,
        label: `${groupVal}`,
        kind: 'folder',
        children: sortedChildItems,
      }
    })
  } else {
    // Default 'Folder' grouping: Prune original tree to matching items
